...
class OsrmRouter implements RoutingInterface
{
    ...
    public function __construct(array $config)
    {
        ...
        $baseUri = rtrim($config['url'] ?? 'https://router.project-osrm.org', '/') . '/';
    ...
    public function route(Coordinate $from, Coordinate $to, array $options = []): ?RouteResult
    {
        ...
        try {
            ...
            // OSRM uses profile names: car, bike, foot
            ...
            $path = "route/v1/{$osrmProfile}/{$from->longitude},{$from->latitude};{$to->longitude},{$to->latitude}";
            ...
            $routeResult = new RouteResult(
                ...
                summary: "Route via OSRM ({$osrmProfile})",
                type: 'route'
            );
